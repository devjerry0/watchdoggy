"""The live-tunable config: every field the web UI can change at runtime,
plus the weekly-window model both schedules share. Structural (restart-
required) fields live in `config.Settings`."""
...
def _labels_from_string(s: str) -> list:
    """The .env forms: a JSON list, or the friendlier comma form ("dog,cat")."""
    if s.startswith("["):
        return json.loads(s)
